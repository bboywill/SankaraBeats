...
function repro_breadcrumb($breadcrumb) {
...
function repro_related_last() {
  static $res = 0;
  $res = $res + 1;
	return (($res == 4) ? ' last' : '' );
}

function repro_popular_last() {
  static $res = 0;
  if ($res == 5) $res = 0;
  $res = $res + 1;
...
function repro_recent_last() {
  static $res = 0;
  $res = $res + 1;
	return (($res == 5) ? ' class="last"' : '' );
}

function repro_comment_last() {
  static $res = 0;
  $res = $res + 1;
	return (($res == 5) ? ' class="last"' : '' );
}

function repro_postgrid_last() {
  static $res = 0;
  if ($res == 8) $res = 0;
  $res = $res + 1;
...
function repro_latestposts_last() {
  static $res = 0;
  $res = $res + 1;
	return (($res == 3) ? ' last' : '' );
}

function repro_latestpostsc_last() {
  static $res = 0;
  $res = $res + 1;
	return (($res == 3) ? ' last' : '' );
}

function repro_slide_img($content, $isout = false) {
  static $res = '';
  static $res1 = 1;
  if ($content) {
...
function repro_cslide_img($content, $isout = false) {
  static $res = '';
  static $res1 = 1;
  if ($content) {
    $res .= '<li class="tab'.$res1.'"><a href="#'.$res1.'">'.$content.'</a></li>';
...
function repro_set_home_video_right($bid, $title, $content, $isout = FALSE) {
  global $base_url;
  static $tabs = array();
  if ($bid) {
    $tabs[$bid]->bid = $bid;
  ...
  if ($isout and isset($tabs) and is_array($tabs)) {
	$out_t = '';
	$out_c = '';
  $out_v = repro_set_home_video_left(FALSE, TRUE);
	$i = 0;
	foreach ($tabs as $data) {
...
function repro_set_home_video_left($content, $isout = false) {
  static $res = '';
  if ($content) {
    $res .= $content;
  }
...
function repro_top($type = 'search') {
...
function repro_set_background($content, $isout = false) {
  static $res = '';
  if ($content) {
    $res = $content;
  }
...
function repro_get_background($bundle, $field_name = 'field_background', $entity_type = 'taxonomy_term') {
  $datadef = unserialize(
...
function repro_get_count_nodes($type = 'video') {
  $n = db_select('node');
...
function repro_user_menu_top($logged_in, $front_page) {
  global $user;
  $output = '';
  ...
  return '<ul class="sf-menu">'.repro_tree_m0().$output .'</ul>';
}

function repro_user_menu_cat() {
  return '<ul class="sf-menu">'.repro_tree_m0('menu-categories').'</ul>';
}

function repro_tree_m0($menu_name = 'main-menu', $type = '') {
  static $menu_output = array();

  if (!isset($menu_output[$menu_name])) {
    $tree = menu_tree_page_data($menu_name);
    $menu_output[$menu_name] = repro_tree_output_m0($tree,$type);
  }
  return $menu_output[$menu_name];
}


function repro_tree_output_m0($tree,$type) {
  $output = '';
  $items = array();
  ...
  foreach ($items as $i => $data) {
    ...
    if ($data['below']) {
	  $output .= '<li><a href="'.url($data['link']['href']).'">'.$data['link']['title'].'</a><ul>'.repro_tree_output_m0($data['below'],$type)."</ul></li>";
    }
    else {
...
function repro_menu_tree($tree) {
  return '<ul>'. $tree['tree'] .'</ul>';
}

function repro_menu_link(array $variables) {
  $element = $variables['element'];
  $sub_menu = '';
  if ($element['#below']) {
...
function repro_menu_item($link, $has_children, $menu = '', $in_active_trail = FALSE, $extra_class = NULL) {
  return '<li>'. $link . $menu ."</li>\n";
}


/* Node */
function repro_get_node($type = 'type') {
	static $node = false;
...
/*
function repro_get_tax_link($vid = 1) {
	$out = '';
...
function repro_truncate_utf8($string, $len, $wordsafe = FALSE, $dots = FALSE, &$ll = 0) {
